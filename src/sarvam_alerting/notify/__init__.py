...
def build_notifiers(
    configs: tuple[NotifierConfig, ...], links: dict | None = None
) -> list[Notifier]:
    notifiers: list[Notifier] = []
    for cfg in configs:
        min_sev = Severity(cfg.min_severity)
        if cfg.type == "console":
            notifiers.append(ConsoleNotifier(min_sev, cfg.streams, links))
        elif cfg.type == "slack_webhook":
            notifiers.append(SlackWebhookNotifier(min_sev, cfg.options, cfg.streams, links))
        elif cfg.type == "slack_bot":
            notifiers.append(SlackBotNotifier(min_sev, cfg.options, cfg.streams, links))
            ...
            notifiers.append(GSheetNotifier(min_sev, cfg.options, cfg.streams, links))
        else:
