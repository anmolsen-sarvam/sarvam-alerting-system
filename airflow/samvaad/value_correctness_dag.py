"""Samvaad value-correctness -- daily LLM check that key values are stated correctly.

Verifies (via transcript) that per-contact values (EMI amount, due date, total due, ...)
are actually conveyed correctly by the agent. Copy to `samvaad/` in airflow-dags.
"""
...
@dag(
    dag_id="samvaad_value_correctness",
    start_date=datetime(2026, 1, 1),
    schedule="0 16 * * *",   # ~21:30 IST, once daily
    catchup=False,
...
def samvaad_value_correctness():
    ...
    def run() -> dict:
        ...
        from sarvam_alerting.reports import build_value_correctness_review
        ...
        print(f"value-correctness: {len(ids)} campaigns, {len(findings)} findings")
        return {"campaigns": len(ids), "findings": len(findings)}

    run()


samvaad_value_correctness()
